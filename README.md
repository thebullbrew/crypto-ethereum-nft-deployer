...
A professional, script-driven toolkit for deploying an ERC-721 NFT collection to Ethereum — from smart contract to deployment to minting. Built on [Hardhat](https://hardhat.org), [OpenZeppelin Contracts](https://openzeppelin.com/contracts), and [viem](https://viem.sh).
...
- **Testnet-first workflow** — rehearse on Sepolia before touching mainnet
- **CI** — GitHub Actions compiles contracts and TypeScript on every push

## Prerequisites

- Node.js 18+
- A Ethereum RPC endpoint (dedicated provider recommended: Alchemy, Infura, QuickNode)
...
> ⚠️ **Always test on Sepolia first.** Point `RPC_URL` at a Sepolia endpoint and fund a throwaway deployer wallet before running against Ethereum mainnet.
...
├── hardhat.config.ts        # Ethereum network (chain id 1)
├── .env.example
...
Deploys `BullBrewNFT` with your collection name, symbol, max supply, per-wallet limit, mint price, and base URI. Prints the contract address and a Etherscan link — save the address as `CONTRACT_ADDRESS` in `.env`.

To verify the contract on Etherscan, use these constructor arguments in order: `COLLECTION_NAME`, `COLLECTION_SYMBOL`, `MAX_SUPPLY`, `MAX_PER_WALLET`, `MINT_PRICE` (in wei), `BASE_URI`.
...
|---|---|
| `RPC_URL` | Ethereum RPC endpoint (Sepolia for testing, mainnet for launch) |
...
- Test the full flow on Sepolia before mainnet.
- Verify the deployed contract on Etherscan before announcing a mint.
